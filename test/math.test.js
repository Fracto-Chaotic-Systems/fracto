...
describe("BigComplex", () => {
  it("preserves decimal precision in arithmetic", () => {
    const result = new BigComplex("0.1", "0.2").add(
      new BigComplex("0.2", "0.3"),
    );
  ...
  it("keeps independent precision contexts and promotes mixed operations", () => {
    const low = new BigComplex("0.1", "0", 20);
    const high = new BigComplex(
      "0.123456789012345678901234567890123456789",
      "0",
      100,
    );
    const result = low.add(high);
    assert.equal(low.precision, 20);
    assert.equal(high.precision, 100);
    assert.equal(result.precision, 100);
    assert.match(result.re.toString(), /^0\.223456789012345678901234567890/);
  });

  it("calculates the principal square root at arbitrary precision", () => {
    const result = new BigComplex("-1", "0", 80).sqrt();
    assert.equal(result.re.toString(), "0");
    assert.equal(result.im.toString(), "1");
  });
});
